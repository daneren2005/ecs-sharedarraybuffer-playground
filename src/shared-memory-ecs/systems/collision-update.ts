...
import type { ComponentSystemCallbacks, ComponentSystemWorld, ComponentTypedArray, EntityUpdateFunction } from '@daneren2005/shared-memory-ecs';
...
import { HEALTH_SHIELDS, HEALTH_TIME_SINCE_DAMAGE } from '../components/health';
import { CONTROLLER_COLOR, CONTROLLER_MONEY } from '../components/controller';
import { CONTROLLED_OWNER } from '../components/controlled';

// Seconds a shield must go without taking damage before it can be hurt again.
const DAMAGE_COOLDOWN = 0.2;
...
interface Blocks {
	position: ComponentTypedArray
	health: ComponentTypedArray
	entity: ComponentTypedArray
	controller?: ComponentTypedArray
	controlled?: ComponentTypedArray
}
...
// any of them by eid.
export const collisionUpdate: EntityUpdateFunction<Components> = (world, entityId, components, queries, callbacks) => {
	const scratch = world as Scratch;
	const velocity = components.velocity;
	if(!velocity || !scratch.spatialIndex || !scratch.spatialData || !scratch.blocksByEid || !scratch.colorByEid) {
		return;
	}
...
collisionUpdate.preRun = (world, entities, queries) => {
	...
	for(let entity of collidable) {
		...
		blocksByEid[entity.entityId] = {
			...
			health,
			entity: entityBlock,
			controller: entity.components.controller,
			controlled: entity.components.controlled
		};
...
function collide(scratch: Scratch, callbacks: ComponentSystemCallbacks<Components>, eid: number, targetEid: number) {
	...
	if(isDead(self)) {
		if(target.controlled) {
			...
			target.controller[CONTROLLER_MONEY] += 1;
		}
	}
}
...
function takeDamage(scratch: Scratch, callbacks: ComponentSystemCallbacks<Components>, eid: number, damage: number) {
	...
	blocks.health[HEALTH_SHIELDS] -= damage;
	blocks.health[HEALTH_TIME_SINCE_DAMAGE] = 0;
	if(blocks.health[HEALTH_SHIELDS] < 0) {
...
function payStation(scratch: Scratch, stationEid: number | undefined, amount: number) {
	...
	if(station?.controller) {
		station.controller[CONTROLLER_MONEY] += amount;
	}
}
...
function canTakeDamage(blocks: Blocks): boolean {
	return blocks.health[HEALTH_TIME_SINCE_DAMAGE] >= DAMAGE_COOLDOWN;
}
